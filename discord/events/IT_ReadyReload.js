...
const targetChannel = '1118279162751356989'; // Replace with the channel ID you want to monitor
...
export default {
    ...
    channel.messages.fetch({ limit: 10 }).then((messages) => {
      messages.forEach((message) => {
        if (message.author.id === targetUser) {
          ...
          console.log(colors.green(`            [=] IT Found Message - ${message.id}`))
        }
      });
    });
    ...
    function removeMessage(message) {
      ...
      console.log(colors.green(`            [=] IT Deleted Message - ${message.id}`))
